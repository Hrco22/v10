...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_03b)
		{
			...
			std::transform(x, x + sizeof(x) / sizeof(x[0]), y.begin(), std::back_inserter(d),
				[](int xi, int yi) { return std::sqrt(xi * xi + yi * yi); });
		...
		TEST_METHOD(test_04a)
		{
			...
			auto res = std::accumulate(std::istream_iterator<double>(ss), std::istream_iterator<double>(), 0.0);// TODO: sum of all values in input stream
		...
		TEST_METHOD(test_04c)
		{
			...
			auto total_age = std::accumulate(v.begin(), v.end(), 0, [](int sum, const person& p)
				{
					return sum + p.age;
		...
		TEST_METHOD(test_05a)
		{
			...
			auto number_of_negative = std::count_if(v.begin(), v.end(), [](int num)
				{
					return num < 0;
				}); // TODO: 
		...
		TEST_METHOD(test_07b)
		{
			...
			std::transform(s.begin(), s.end(), s.begin(), [](char c) {
				if (std::strchr("aeiouAEIOU", c) != nullptr) {
					return 'x';
				}
				return c;
				});
